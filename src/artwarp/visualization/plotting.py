...
def plot_reference_contours(
    ...
    for cat_idx in range(num_categories):
        ...
        if len(valid_contour) > 0:
            ...
            ax.text(0.5, 0.5, 'Empty Category', 
...
def plot_convergence_history(
    ...
    ax.plot(iterations, reclassifications, 'o-', linewidth=2, 
           markersize=6, color='#2E86AB', label='Reclassifications')
    
    # mark convergence point if converged
    if results.converged:
        conv_iter = results.num_iterations
        ax.axvline(conv_iter, color='green', linestyle='--', linewidth=2,
...
def plot_contours_by_category(
    ...
    if len(category_indices) == 0:
        ax.text(0.5, 0.5, f'No contours in category {category_id}',
               ha='center', va='center', fontsize=14)
        return fig
...
def plot_match_distribution(
    ...
    if len(matches) == 0:
        ax.text(0.5, 0.5, 'No categorized samples',
               ha='center', va='center', fontsize=14)
        return fig
    
    # Create histogram
    n, bins_edges, patches = ax.hist(matches, bins=bins, alpha=0.7, 
                                     color='#2E86AB', edgecolor='black')
    
    # mean line
    mean_match = float(np.mean(matches))
    ax.axvline(mean_match, color='red', linestyle='--', linewidth=2,
              label=f'Mean: {mean_match:.1f}%')
    
    # median line
    median_match = float(np.median(matches))
    ax.axvline(median_match, color='green', linestyle='--', linewidth=2,
...
def create_results_report(
    ...
    for cat_id in sorted(results.get_category_sizes().keys()):
        ...
        fig = plot_contours_by_category(
            contours, results.categories, cat_id,
            contour_names, ref_contour
        )
...
def _plot_reference_contours_axes(ax: Axes, weight_matrix: NDArray[np.float64]) -> None:
    ...
    for cat_idx in range(num_categories):
        ...
        if len(valid_contour) > 0:
            time_points = np.arange(len(valid_contour))
            ax.plot(time_points, valid_contour, linewidth=2,
                   color=colors[cat_idx], label=label, alpha=0.7)

    ax.set_xlabel('Time Point', fontweight='bold')
    ax.set_ylabel('Frequency (Hz)', fontweight='bold')
    title = f'Reference Contours ({num_categories} categories)' if num_categories > max_legend_cats else 'Reference Contours'
...
def _plot_category_table_axes(
    ax: Axes,
    results: TrainingResults,
    contour_names: Optional[List[str]]
) -> None:
...
def _plot_match_by_category_axes(ax: Axes, results: TrainingResults) -> None:
    ...
    if data_by_category:
        ...
        parts = ax.violinplot(data_by_category, positions=range(n_plot),
        ...
        if n_plot > 24:
            ...
            labels = [str(cat) if i % step == 0 else '' for i, (_i, cat) in enumerate(positions_cats)]
