...
def load_ctr_file(filepath: Path) -> Dict[str, Any]:
    ...
    Returns:
        Dictionary containing:
            ...
            - 'id': UUID string of this contour/prototype (if present)
            - 'parent_ids': List of UUID strings of contributing input contours
              (if present, from reference contours saved by ARTwarp MATLAB stable-1)
    ...
    Note:
        ...
        Reference contours generated by ARTwarp (MATLAB) stable-1 (post-Feb 2026)
        additionally store 'id' and 'parent_ids' for provenance tracking.
    """
    try:
    ...
    try:
        ...
        # provenance fields present in MATLAB ARTwarp stable-1 reference contours
        contour_id: Optional[str] = None
        raw_id = mat_data.get("id", None)
        if raw_id is not None:
            try:
                contour_id = str(raw_id).strip()
            except Exception:
                contour_id = None

        parent_ids: Optional[List[str]] = None
        raw_parent_ids = mat_data.get("parent_ids", None)
        if raw_parent_ids is not None:
            try:
                arr = np.atleast_1d(raw_parent_ids)
                parent_ids = [str(p).strip() for p in arr if str(p).strip()]
            except Exception:
                parent_ids = None
