...
def load_mat_categorisation(filepath: str) -> Dict[str, Any]:
    ...
    if 'NET' not in mat:
        raise ValueError(f"File {filepath} does not contain 'NET' (not an ARTwarp categorisation file)")
    ...
    def _get_field(obj, name: str, default: Any = None) -> Any:
        ...
        if hasattr(obj, 'dtype') and hasattr(obj.dtype, 'names') and obj.dtype.names and name in obj.dtype.names:
            val = obj[name]
